...
if nd is None:
    class ASTGCN(nn.Module):
        def __init__(self, args):
            super().__init__()
            self.feature_dim = getattr(args, 'baseline_input_dim', 1)
            self.output_len = args.output_len
            self.output_dim = args.output_dim
            hidden = getattr(args, 'astgcn_hidden_dim', 64)
            self.input_proj = nn.Linear(self.feature_dim, hidden)
            self.temporal_attn = nn.MultiheadAttention(hidden, num_heads=4, batch_first=True)
            self.spatial_score = nn.Linear(hidden, hidden, bias=False)
            self.temporal_conv = nn.Conv2d(hidden, hidden, kernel_size=(1, 3), padding=(0, 1))
            self.output = nn.Linear(args.input_len * hidden, self.output_len * self.output_dim)

        def forward(self, source):
            x = source[..., :self.feature_dim]
            batch_size, input_len, num_nodes, _ = x.shape
            x = self.input_proj(x)

            temporal_x = x.permute(0, 2, 1, 3).reshape(batch_size * num_nodes, input_len, -1)
            temporal_x, _ = self.temporal_attn(temporal_x, temporal_x, temporal_x)
            x = temporal_x.reshape(batch_size, num_nodes, input_len, -1).permute(0, 2, 1, 3)

            node_repr = x.mean(dim=1)
            attn = torch.softmax(torch.matmul(self.spatial_score(node_repr), node_repr.transpose(1, 2)), dim=-1)
            x = torch.einsum('bnm,btmh->btnh', attn, x)

            x = self.temporal_conv(x.permute(0, 3, 2, 1)).permute(0, 3, 2, 1)
            x = x.permute(0, 2, 1, 3).reshape(batch_size, num_nodes, input_len * x.shape[-1])
            x = self.output(x).view(batch_size, num_nodes, self.output_len, self.output_dim)
