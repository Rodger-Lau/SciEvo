...
class ODEGCN(nn.Module):
    ...
    def __init__(self, num_nodes, num_features=None, num_timesteps_input=None,
                 num_timesteps_output=None, A_sp_hat=None, A_se_hat=None):
        """ 
        Args:
            num_nodes : number of nodes in the graph
        ...
        if not isinstance(num_nodes, int):
            ...
            num_features = getattr(args, 'baseline_input_dim', 1)
            num_timesteps_input = args.input_len
            num_timesteps_output = args.output_len
            A_sp_hat = getattr(args, 'A_sp_hat', None)
            A_se_hat = getattr(args, 'A_se_hat', None)
        if A_sp_hat is None:
            A_sp_hat = torch.eye(num_nodes)
        if A_se_hat is None:
            A_se_hat = torch.eye(num_nodes)
    ...
    def forward(self, x):
        ...
        """
        x = x[..., :self.num_features].permute(0, 2, 1, 3)
        device = x.device
        for block_group in list(self.sp_blocks) + list(self.se_blocks):
            for block in block_group:
                block.A_hat = block.A_hat.to(device)
                block.odeg.odeblock.odefunc.adj = block.odeg.odeblock.odefunc.adj.to(device)
        outs = []
        # spatial graph
        ...
        return self.pred(x).permute(0, 2, 1).unsqueeze(-1)
