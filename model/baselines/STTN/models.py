...
class STTN(nn.Module):
    def __init__(self, args):
        ...
        hidden = getattr(args, 'sttn_hidden_dim', 64)
        self.input_proj = nn.Linear(self.feature_dim, hidden)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=hidden,
            nhead=getattr(args, 'sttn_heads', 4),
            dim_feedforward=getattr(args, 'sttn_ffn_dim', 128),
            dropout=args.dropout,
            batch_first=True,
        )
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=getattr(args, 'sttn_layers', 2))
    ...
    def forward(self, source):
        ...
        x = x.permute(0, 2, 1, 3).reshape(batch_size * num_nodes, input_len, self.feature_dim)
        x = self.input_proj(x)
        x = self.encoder(x)
        x = x.reshape(batch_size, num_nodes, input_len * x.shape[-1])
