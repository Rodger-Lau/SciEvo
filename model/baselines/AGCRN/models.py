...
class AGCRN(nn.Module):
    def __init__(self, args):
        ...
        self.input_dim = getattr(args, 'baseline_input_dim', 1)
        self.hidden_dim = getattr(args, 'rnn_units', 64)
        self.output_dim = args.output_dim
        self.horizon = args.output_len
        self.num_layers = getattr(args, 'num_layers', 1)

        embed_dim = getattr(args, 'embed_dim', 10)
        cheb_k = getattr(args, 'cheb_k', 2)
        self.node_embeddings = nn.Parameter(torch.randn(self.num_node, embed_dim), requires_grad=True)

        self.encoder = AVWDCRNN(args.num_nodes, self.input_dim, self.hidden_dim, cheb_k,
