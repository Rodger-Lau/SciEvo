...
class GWNET(nn.Module):
    def __init__(self, args, device, dim_in, dim_out):
        ...
        self.adj_mx = getattr(args, 'adj_mx', None)
        self.num_nodes = args.num_nodes
        self.feature_dim = dim_in

        self.dropout = args.dropout
        self.blocks = getattr(args, 'blocks', 2)
        self.layers = getattr(args, 'layers', 2)
        self.gcn_bool = getattr(args, 'gcn_bool', False)
        self.addaptadj = getattr(args, 'addaptadj', True)
        self.adjtype = getattr(args, 'adjtype', 'identity')
        self.randomadj = getattr(args, 'randomadj', True)
        self.aptonly = getattr(args, 'aptonly', False)
        self.kernel_size = getattr(args, 'kernel_size', 2)
        self.nhid = getattr(args, 'nhid', 32)
        self.residual_channels = getattr(args, 'residual_channels', 32)
        self.dilation_channels = getattr(args, 'dilation_channels', 32)
        self.skip_channels = self.nhid * 8
        self.end_channels = self.nhid * 16
        self.input_window = args.input_len
        self.output_window = args.output_len
    ...
    def forward(self, source, select_dataset=None):
        inputs = source[..., :self.feature_dim]
        ...
        if in_len < self.receptive_field:
            inputs = nn.functional.pad(inputs, (self.receptive_field - in_len, 0, 0, 0))
