...
def save_experiment_results(result_dir, experiment_idx, dataset_name, backbone_name,
    ...
    append_csv_row(
        ...
        fieldnames=[
            ...
            'test_acc', 'best_val_acc', 'final_train_loss', 'final_val_loss',
...
def train_model_RL(model, train_loader, val_loader, test_loader, model_name, logger, cur_mean, cur_std, dqn_agent, action_scale_map, H_matrix, rl_optimizer, batch_size, num_classes, write=False, common=True, n_epochs=50, new_domain=False, acc=0):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for batch_X, batch_y in train_loader:
            ...
            # print(f'batch_y shape:{batch_y.shape}') # batch_y shape:torch.Size([80])
...
def main():
    ...
    test_loaders = []
    model_name = 'BrainAI'
    OUTPUT_DIR = f"./EEG_output_BCI2a/{model_name}/{backbone_name}_results_classification"
    BATCH_SIZE = 80
    N_EPOCHS = 800
    N_SUBJECTS = 9
    new_i = 8
    ...
    for subject_id in range(N_SUBJECTS):
        DATA_PATH = f"./data/data/bci2a_processed/subject_{subject_id+1:02d}_preprocessed.npz"
        logger.info(f"\n1. 加载 BCI2a 预处理数据 A{subject_id+1:02d}...")
        try:
            X_train, y_train, X_valid, y_valid, X_test, y_test, cur_mean, cur_std = load_preprocessed_data(DATA_PATH)
    ...
    # =========================
    # 3. Repeated experiments. Each saved result is final target-domain performance.
    # =========================
    for experiment_idx in range(num_experiments):
        ...
        # ============================================================
        # 3.1 Source subject individual training and gradient computation
        # ============================================================
        ...
        logger.info(f'======================new domain starts: {target_subject_name}=============================')
        ...
        logger.info("\n" + "=" * 60)
        logger.info(f"==============================第{experiment_idx+1}/{num_experiments}次实验结果汇总")
        logger.info("=" * 60)
