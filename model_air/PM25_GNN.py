...
class GraphGNN(nn.Module):
    def __init__(self, device, edge_index, edge_attr, in_dim, out_dim, wind_mean, wind_std, p0=0.1):
...
class PM25_GNN(nn.Module):
    def __init__(self, hist_len, pred_len, in_dim, city_num, batch_size, device, edge_index, edge_attr, wind_mean, wind_std, out_dim=1, p0=0.1, task_index=0):
    ...
    def forward(self, pm25_hist, feature):
        self.graph_gnn.p = self.p
        total_activate_freq = 0
        pm25_pred = []
        current_batch_size = pm25_hist.size(0)
        h0 = torch.zeros(current_batch_size * self.city_num, self.hid_dim).to(self.device)
        hn = h0
        xn = pm25_hist[:, -1]
        for i in range(self.pred_len):
            ...
            # print(f'xn_gnn shape:{xn_gnn.shape}')
            xn_gnn = self.graph_gnn(xn_gnn)
            x = torch.cat([xn_gnn, x], dim=-1)

            hn = self.gru_cell(x, hn)

            xn = hn.view(current_batch_size, self.city_num, self.hid_dim)
            # print(f'before xn shape:{xn.shape}')
            # before xn shape:torch.Size([32, 1, 64])
            # after xn shape:torch.Size([32, 1, 1])
            count = (xn > 0).sum().item()
            size = 1
            for j in xn.shape:
                size *= j
            ...
            xn = self.fc_out(xn)
            # print(f'after xn shape:{xn.shape}')
            # if self.task_index == 1:
