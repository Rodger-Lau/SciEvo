...
dataset="NYC"
target_source_idx=0       # CROWDIN
target_task_idx=0         # 00:00-04:00 when task_per_dir=6
task_per_dir=6
result_group="baseline_repeats_NYC_CROWDIN_0_4"
plot_dir="csv_files/BrainAI/${dataset}/${result_group}"
mkdir -p "outputs/BrainAI/${dataset}" "$plot_dir"

echo "NYC/CROWDIN/00:00-04:00 repeated baseline experiments"
echo "Models : ${models[*]}"
echo "Seeds  : ${seeds[*]}"
...
for model in models:
    for run_number, seed in enumerate(seeds, start=1):
        ...
        summary_path = Path("csv_files/BrainAI/NYC") / exp_name / "summary.csv"
...
fig.suptitle("NYC / CROWDIN / 00:00-04:00 baseline repetitions")
fig.tight_layout()
