...
from models.GIN_HIV import GAT


class HIVGINE(nn.Module):
    """Edge-aware backbone for HIV molecular graphs."""
    ...
    def _get_edge_attr(self, data, edge_index, x):
        edge_attr = getattr(data, "edge_attr", None)
        if edge_attr is None:
            return torch.zeros(edge_index.size(1), self.edge_dim, device=x.device, dtype=x.dtype)
    ...
    def forward(self, data, activate_num=0):
        ...
        edge_attr = self._get_edge_attr(data, edge_index, x)
